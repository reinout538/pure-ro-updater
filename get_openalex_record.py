...
class getOpenalex():
    ...
    def get_green_url (self, json_openalex):
        ...
        for loc in json_openalex['locations']:
            ...
            #exclude published version and PMC, DOAJ, VU Research Portal locations
            if loc['is_oa'] == True and loc['landing_page_url'] != None and loc['version'] != 'publishedVersion' and source_id != 'https://openalex.org/S4306401107':
                check_url = requests.get(loc['landing_page_url'])
                if check_url.ok == False:
                    continue
