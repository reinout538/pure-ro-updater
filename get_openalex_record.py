...
class getOpenalex():
    ...
    def get_green_url(self, json_openalex):
        ...
        for loc in json_openalex['locations']:
            source_id = (loc.get('source') or {}).get('id')
            
            if (loc['is_oa'] == True 
                    and loc['landing_page_url'] != None 
                    and loc['version'] != 'publishedVersion' 
                    and source_id != 'https://openalex.org/S4306401107'):
                
                if any(excl in loc['landing_page_url'] for excl in excluded):
                    continue
                
                try:
                    check_url = requests.get(loc['landing_page_url'], timeout=10)
                    if not check_url.ok:
                        continue
        ...
        for candidate in candidates:
            ...
            # Fix: use 'any()' or check each substring separately
            if ('handle.net' in candidate['landing_page_url'] or 'urn.' in candidate['landing_page_url']) \
                    and 'handle.net/1871' not in candidate['landing_page_url']:
                break
...
DOI_list = ["10.1177/15459683211000730"]
...
for n, DOI in enumerate(DOI_list[0:]):
    ...
    print (openalex.green_url_landing)    
