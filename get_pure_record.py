...
class getPure():
     ...
     def __init__(self, UUID):
        ...
        if self.status == 200:
             ...
             self.scopus_eid = self.get_eid (json_pure_pub)
               
        else:
             self.pure_id = self.type = self.category = self.peer_review = self.electr_versions = self.curr_pub_status = self.print_year = self.print_month = self.print_day = self.online_year = self.online_month = self.online_day = self.managing_org = self.doi = self.doi_index = self.doi_access = self.doi_license = self.keyw_list = self.class_keyw = self.journal_uuid = self.journal_issn = self.workflow = self.scopus_eid = None     
     ...
     def get_eid(self, json_pure_pub):
...
publ_uuids = ["4564465a-ece3-42b1-b3a6-41554cb08d3c"]
