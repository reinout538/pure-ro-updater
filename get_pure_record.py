...
def get_pure(UUID, base_url, crud_api_key):

     #get pure record log directory
     response_get_pub = requests.get(base_url+'/ws/api/research-outputs/'+UUID, headers={'Accept': 'application/json', 'Content-Type': 'application/json', 'api-key':crud_api_key})
...
class getPure():
     ...
     def __init__(self, UUID, base_url, crud_api_key):
          
          pure_record = get_pure(UUID, base_url, crud_api_key)        
          json_pure_pub = pure_record[0]
          self.status = pure_record[1]
          if self.status == 200:
             ...
             self.template = json_pure_pub['type']['uri'].strip("/").split("/")[5]
